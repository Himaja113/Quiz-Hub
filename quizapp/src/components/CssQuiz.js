...
function Quiz() {
  ...
  const [shuffledQuestions, setShuffledQuestions] = useState(CssData); // Shuffled data

  useEffect(() => {
    if (!showResult) { // Shuffle questions only before showing result
      setShuffledQuestions(shuffleArray(CssData)); // Shuffle on component mount
    }
  ...
  const resetAll = () => {
    ...
    setSelectedOptions([]);
    setScore(0);
    setShuffledQuestions(shuffleArray(CssData)); // Re-shuffle on reset
  };
